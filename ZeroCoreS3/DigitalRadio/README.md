...
- Version: `0.1.1-web-beta`. The local interface and HTTP error messages are in English.
- Firmware source: [`Digital-Radio-for-Raspberry-Pi` commit `97c66ee`](https://github.com/RASPIAUDIOadmin/Digital-Radio-for-Raspberry-Pi/commit/97c66ee0959ce0b9cdcda4c51b8aff6ac22d3406), PlatformIO environment `zerocore_s3_web`.
- `corezero-digital-radio-0.1.1.factory.bin` was assembled with `esptool --chip esp32s3 merge_bin` from the PlatformIO bootloader at `0x0`, partition table at `0x8000`, `boot_app0.bin` at `0xE000`, and the radio application at `0x10000`; flash mode DIO, 80 MHz, 16 MB.
- The prior source application was validated on an assembled ZeroCore S3 + shield with FM/DAB tuning, analog audio, local web controls, Wi-Fi persistence and hotspot fallback. This English text update compiled successfully; the merged factory image was checked against its components and SHA-256 digest, but was not flashed over the preserved `app0` of that prototype.

...
void I2SAudioSpeaker::speaker_task(void *params) {
  ...
  if (!this_speaker->send_esp_err_to_event_group_(this_speaker->start_i2s_driver_(audio_stream_info))) {
    ...
    while (this_speaker->pause_state_ || !this_speaker->timeout_.has_value() ||
      ...
      // FutureProof exposes the same three speaker choices through its TAS2780.
      // Voice DSP+ performs the selection in PCM so the headphone path can stay
      // true stereo while the differential onboard speaker receives a safe mono
      // signal. Averaging L+R leaves 6 dB of summing headroom.
      const uint8_t output_channel_mode =
          this_speaker->output_channel_mode_.load(std::memory_order_relaxed);
      if (bytes_read > 0 && audio_stream_info.get_channels() == 2 &&
          output_channel_mode != I2SAudioSpeaker::OUTPUT_STEREO) {
        const size_t bytes_per_sample = audio_stream_info.samples_to_bytes(1);
        const size_t bytes_per_frame = bytes_per_sample * 2;
        const size_t frame_count = bytes_read / bytes_per_frame;
        for (size_t frame = 0; frame < frame_count; ++frame) {
          uint8_t *left_data = &this_speaker->data_buffer_[frame * bytes_per_frame];
          uint8_t *right_data = left_data + bytes_per_sample;
          const int32_t left = audio::unpack_audio_sample_to_q31(left_data, bytes_per_sample);
          const int32_t right = audio::unpack_audio_sample_to_q31(right_data, bytes_per_sample);
          int32_t mono = left;
          if (output_channel_mode == I2SAudioSpeaker::OUTPUT_MONO_MIX) {
            mono = static_cast<int32_t>((static_cast<int64_t>(left) + static_cast<int64_t>(right)) / 2);
          } else if (output_channel_mode == I2SAudioSpeaker::OUTPUT_RIGHT_ONLY) {
            mono = right;
          }
          audio::pack_q31_as_audio_sample(mono, left_data, bytes_per_sample);
          audio::pack_q31_as_audio_sample(mono, right_data, bytes_per_sample);
        }
      }

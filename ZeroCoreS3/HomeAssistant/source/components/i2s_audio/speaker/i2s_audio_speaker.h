...
class I2SAudioSpeaker : public I2SAudioOut, public speaker::Speaker, public Component {
 public:
  ...
  void set_output_channel_mode(uint8_t mode) {
    if (mode > OUTPUT_STEREO)
      mode = OUTPUT_MONO_MIX;
    this->output_channel_mode_.store(mode, std::memory_order_relaxed);
  }
 ...
 protected:
  ...
  // The onboard speaker is mono, while the headphone route stays stereo.
  std::atomic<uint8_t> output_channel_mode_{OUTPUT_MONO_MIX};

...
VoiceDSPI2SMicrophone = i2s_audio_ns.class_(
    "VoiceDSPI2SMicrophone", I2SAudioIn, microphone.Microphone, cg.Component
)
...
def _supported_voice_dsp_settings(config):
    if config[CONF_ADC_TYPE] == "internal":
        raise cv.Invalid("Internal adc_type is not supported for the Voice DSP+ microphone integration.")
    if config[CONF_PDM] :
        raise cv.Invalid("PDM is not supported for the Voice DSP+ microphone integration.")
    if config[CONF_BITS_PER_SAMPLE] != 32:
        raise cv.Invalid("I2S must be 32-bit for the Voice DSP+ microphone integration.")
